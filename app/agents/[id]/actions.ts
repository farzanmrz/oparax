...
// default `"page"`: DeskControls lives in the shared desk layout (rendered on Feed, Skipped,
...
/** Pause a desk: Oparax stops watching the beat. */
...
/**
 * Delete a desk. `drafts.agent_id` carries `ON DELETE CASCADE`, as do the other desk-owned
 * tables (stories, story_assignments, excluded_posts, source_configs), so the database cleans
 * them up. `source_posts` is deliberately untouched: it is genuinely cross-desk (one ingested
 * post can feed several desks), so it is not this desk's to delete. On success it redirects
