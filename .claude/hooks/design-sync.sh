...
# DESIGN.md changes, it knows to sync it to Claude Design"). The fingerprint covers the deliberate
# design system only: DESIGN.md, the theme tokens and the synced bundle. Components added with
# `shadcn add` take the theme by themselves and are not the design system, so they never trigger it.
# design-system/.synced holds the fingerprint of the last good sync.
...
current=$(
  {
    ...
    find design-system -type f ! -name .synced -print0 | sort -z | xargs -0 cat
