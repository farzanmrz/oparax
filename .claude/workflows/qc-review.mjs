export const meta = {
  ...
  whenToUse: "feature-qc's review pass — replaces the serial /simplify then /code-review passes with a single barrier so all finders run at once. The session applies fixes and delta-verifies afterward.",
  phases: [
    { title: 'Review', detail: 'cleanup + conventions on sonnet, three bug angles on opus — all concurrent' },
  ],
}
...
// adjudicates (plan-frozen vetoes win), applies, then delta-verifies.
...
const FINDERS = [
  ...
  {
    class: 'bug', angle: 'line-by-line', agentType: 'bug-finder', model: 'opus',
