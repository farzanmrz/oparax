...
This skill is single-source: Codex invokes this same file (`$feature-ship`,
...
* **After each batch of fixes:** re-run `feature-lint` + the boot smoke, and
...
* **Standing pre-approval carve-out:** when the owner's own invocation
  already says to ship ("/feature-ship", "ship it", "close the slice"), that
...
```bash
.claude/skills/feature/scripts/promote.sh beta main
```
...
* **Feature slices always run on `ft/<issue#>`:** app code is never developed
  directly on `beta` or `main` (owner-directed instruction-file/doc
