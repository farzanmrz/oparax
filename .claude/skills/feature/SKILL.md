...
When the command names an issue (`/feature 140`, `/feature #140`, or "feature issue 140"), that issue's body is the owner's brief. Before anything else run `gh issue view <N> --json number,title,body` and treat the body exactly as if the owner had typed it: preserve it as his original words for the pair protocol, take what it lists under decided as given, and bring what it lists as open to the step 1 discussion as the questions to settle. Read the documents it names under "Read first" before proposing scope. The owner should not have to restate anything the brief says. At step 8 create nothing new: run `bash .claude/scripts/start.sh --issue <N> .feature/issue-body.md`, which replaces the brief with the approved plain plan on that same issue and cuts `ft/<N>`. The brief is consumed by design; `docs/roadmap.md` and `docs/onboarding-algorithm.md` remain the permanent record.
