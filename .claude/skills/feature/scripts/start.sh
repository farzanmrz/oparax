#!/usr/bin/env bash
# Close the plan gate: open an issue and cut ft/<issue> from fetched origin/dev.
#
# Usage:
...
#
# There is deliberately no direct-dev mode and no persisted run state. A slice is
# identified by its branch, which is the only marker QC needs (`origin/dev...ft/<N>`);
# the release target is passed to ship.sh at ship time rather than saved here.
set -euo pipefail
