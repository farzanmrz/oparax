...
# range's changed files — residual lint is feature-fix/-lint's job, not a gate.
#
...
if [ -n "$changed" ]; then
  ...
  if [ "$lint_rc" -eq 0 ]; then
    ...
    echo "LINT residual: findings present (~$n lines; not a gate — feature-fix/-lint owns them)"
    printf '%s\n' "$lint_out" | tail -5
  fi
else
