...
1. Write `.feature/lanes/qc.brief`, in this order:
   ...
   - Two skills consult lines built from the plan's `Skills:` line: one for the two Codex lanes in Codex form (`$vercel:<name>`, `$supabase:<name>`, `$posthog:<name>`, `$use-railway`, and `$<name>` for the global skills `frontend-design`, `web-design-guidelines`, `ai-elements` and `framer-motion-animator`; drop `ui-ux-pro-max`), phrased "Codex lanes: consult these skills where a finding rests on a rule they cover, and cite the rule: ..."; one for grok and agy in bare names, phrased "Grok and agy: these are rules to weigh, not skills you can invoke: ...".
