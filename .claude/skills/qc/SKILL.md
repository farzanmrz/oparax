...
2. Launch three lanes, each with `bash .claude/scripts/lane.sh start <lane> -- <cmd>` (foreground; lane.sh detaches and returns at once). Same readers as the plan critique minus agy; QC wall time is pinned to the slowest lane — the codex lanes land in a couple of minutes (measured 2026-08-18 at QC: sol 76s, terra 52s), and the grok lane's prompt allows about 10 minutes, so budget QC wall on grok, worst case the 15-minute LANE_HUNG_SECONDS wall. agy is NOT a QC lane: on 2026-08-18 it crashed on its own at QC (a wrong-filename read became a fatal CLI error after 276s, one turn, no findings) while working at critique the same day; it stays a critique lane only:
   ...
   **ONE-ROUND CALIBRATION OVERRIDE (owner decision 2026-08-23, part of the #128 experiment; applies ONLY to the FIRST QC round of issue #126, then this whole block is deleted — the round's closing message reminds the owner).** For that one round, instead of the three lanes above, launch NINE, same brief, same launch mechanics, one background wait per lane (names must contain codex/grok/agy for the extractor):
   - `qc-codex-sol-hi` and `qc-codex-sol-med`: the sol command at `model_reasoning_effort=high` and `=medium`.
   - `qc-codex-terra-hi` and `qc-codex-terra-med`: the terra command at high and medium.
   - `qc-agy-pro` and `qc-agy-flash`: `agy --model=gemini-3.1-pro-high --effort=high` and `agy --model=gemini-3.7-flash-high --effort=high`, both with `--output-format=json --print-timeout=15m --print="Read $PWD/.feature/lanes/qc.brief and follow it exactly."` — agy's QC ban (the 2026-08-18 crash) is LIFTED for this one round only, precisely to re-test it; if either crashes again the ban stands confirmed.
   - `qc-grok-hi-10m` and `qc-grok-med-10m`: the grok command at `--effort high` and `--effort medium`, each with its own prompt file carrying the standard 10-minute clock budget.
   - `qc-grok-hi-uncap`: the grok command at high with a prompt file that carries NO time language at all (only the pointer line and the reading ceiling) — the untested cell; if it runs to the 900s wall and dies, that outcome IS the data point, kill and classify normally. Adjudicate all nine under the unchanged state table, evidence rules, and dispositions rigor — calibration multiplies findings including weak-cell false positives, and at QC a bad finding becomes a bad code change, so nothing enters the fix list without its citation verified. The calibration deliverable is a FILE, never the owner's message: write `.feature/qc-calibration-126.md` with one row per lane — elapsed time in MM:SS, findings count, findings surviving dispositions, state (incl. any crash or wall death) — plus one line naming which findings were unique to a single lane. The closing message to the owner stays EXACTLY the standard step-8 shape (outcome, fix list or clean, next command) with one added sentence: "Calibration data from this round is in .feature/qc-calibration-126.md for the analysis session." No timings, counts, or lane mechanics in the owner's message beyond that sentence.

   Then (standard rounds) arm three separate background waits, one Bash call each with `run_in_background: true`: `LANE_HUNG_SECONDS=900 bash .claude/scripts/lane.sh waitall qc-codex-sol`, then the same for `qc-codex-terra`, `qc-grok`. `LANE_HUNG_SECONDS=900` is the 15-minute terminal wall cap (owner decision 2026-08-23): a lane still running at 15 minutes makes its wait print `HUNG`, which the state table below turns into a kill, a `--timed-out` extraction, and at most one resume attempt. Do not wait on any of them in the foreground; the session is re-invoked as each exits. Never edit `lane.sh` while a wait runs.
