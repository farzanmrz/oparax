...
description: >-
  ...
  Codex fix build itself without waiting, on Astra High. No Workflow tool.
...
Never start a dev server, never run pnpm dev, never touch env files, never open a browser or use any browser/computer-use tool, never write to git except the one `gates:` commit above and the step-1 `meta:` commit for `.claude/` and `.codex/` files. The one exception is the screenshot check in 4a, which the owner allowed on September 23. This binds the command while it runs; if the owner asks in their own words in the chat to run the app or open a browser, that wins immediately (AGENTS.md).

### 4a. The screenshot check (owner, September 23)

When the slice touches a screen, one background subagent (`general-purpose`, this session's model) takes pictures and nothing else: it starts the app in the background on a free port (never 3000, never the owner's server), opens each page the plan's acceptance journeys name with `agent-browser` (installed on this machine, runs off-screen), waits for the page to settle, screenshots it at 1280px and 390px wide in dark and in light, saves the images under `.feature/lanes/qc-shots/<page>-<width>-<scheme>.png`, then stops the app it started. It never clicks through a journey, never signs in, never fills a form, and never triggers a paid model call: the pages render from data already in the database, and a page that needs a build to exist is reported as "not screenshotted" with the reason. Timing: at most three minutes in total; a page that has not settled in 30 seconds is skipped and named. The agent returns the list of files and the console errors it saw, nothing more.

The QC session then reviews the images itself with `design-review` and `accessibility` loaded, against the Claude Design export in `.feature/` when the slice has one and `DESIGN.md` otherwise: layout at both widths, contrast in both schemes, states the plan named that the page does not show, anything that reads as a broken build. Each finding goes into `.feature/lanes/qc-claude.findings.json` in the lane contract shape, with `file` set to the page's route file and `line` null. A missing screenshot is never a finding against the product.
...
1. Write `.feature/lanes/qc.brief`, in this order:
   ...
   - The skill files. Before writing the brief, copy every skill the plan's `Skills:` line names into `.feature/lanes/skills/<name>/` (the whole skill folder, resolved from `~/.agents/skills`, `~/.claude/skills`, the project's `.claude/skills`, the plugin cache under `~/.claude/plugins/cache`, or Codex's plugin cache under `~/.codex/plugins/cache`, where the Stripe skills live), so every reviewer, whatever its harness, reads the same rules the builders followed (owner, September 23: reviewers cannot critique fairly without the builders' knowledge). The brief says: "Read every SKILL.md under .feature/lanes/skills/ before judging; where a finding rests on a rule from one of them, cite the rule." Reviewers get no connectors or MCPs: they judge the plan and the code, not live systems.
   ...
   - Two skills consult lines built from the plan's `Skills:` line: one for the two Codex lanes in Codex form (`$vercel:<name>`, `$supabase:<name>`, `$posthog:<name>`, and `$<name>` for the global skills `frontend-design`, `web-design-guidelines`, `accessibility`, `beautiful-shadows`, `emil-design-eng`, `design-review` and `ai-elements`), phrased "Codex lanes: consult these skills where a finding rests on a rule they cover, and cite the rule: ..."; one for grok, agy and the Cursor lanes in bare names, phrased "Grok, agy and Cursor lanes: these are rules to weigh, not skills you can invoke: ...".
