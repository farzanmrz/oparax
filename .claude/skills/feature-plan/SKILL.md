...
The slice comes from the user's ask — never from `docs/triage.md`. Scratch lives in
`docs/feature/` (create self-gitignoring: `mkdir -p docs/feature && printf '*\n' >
docs/feature/.gitignore`).

**Clear the thinking first.** Rambling/multi-directional ask → interview one
question at a time with your best guess attached. Direction genuinely unknown →
invoke `idea-refine` (save path override: `docs/feature/`). Clear ask → skip both.
...
ask; save its spec+plan verbatim to `docs/feature/spec-plan.md`. Gate revisions
...
`.claude/skills/feature/scripts/start.sh "<feature name>" docs/feature/spec-plan.md`
