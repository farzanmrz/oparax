...
- **Tool sync** — a diff changing `lib/agent/tools.ts` or any tool's `inputSchema` updates `desk-agent.md` in the same commit. The agent's tool set is a fixed object literal (three tools, no sentinel/registry mechanism) — the prompt's tool list must name exactly those three and no others.
