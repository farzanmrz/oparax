...
- Default-tool scoping via `disableTool()` sentinels: `bash`/`read_file`/`write_file`/`glob`/`grep` (shell/FS — security), `web_search` (deferred + ~$5/1k), `agent` (subagent — unneeded), `todo` (task list — unneeded) are all OFF. Enabled defaults: `web_fetch` (user URLs) and `ask_question`. eve has no central disable list — per-file sentinels are the only mechanism, and an enabled default has **no** file (files only disable/override). `load_skill`/`connection_search` aren't present at all (register only if you declare eve skills/connections).
