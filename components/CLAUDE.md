...
- `ui/` — stock shadcn, installed via CLI (`pnpm dlx shadcn@latest add <name>`; the CLI reads root `components.json` for style/aliases/css entry). Skill: `vercel:shadcn`. Never hand-edit; re-add/regenerate via the CLI instead.
