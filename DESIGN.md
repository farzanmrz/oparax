...
The feed card is one 10px gradient card with three stacked zones: a source-tinted strip, the story body, and a blue-tinted draft zone. The source strip carries the source identity, freshness, alert slot, and menu; its leading icon is a fixed 15px slot for every source kind — the X logo, a website's favicon (rounded 3px, falling back to the generic globe when a site serves none), or the globe — so the strip's leading edge never shifts. Website sources display their onboarded publication name ("Mundo Deportivo") and fall back to the www-stripped hostname; the story body carries only the title and synthesis; the draft zone carries the editable post text, count and edited state, and posting control. The draft editor height follows rendered content and reflows with width and breakpoint changes, with no clipping, overlap, reserved textarea floor, or spacer. When unfocused, the draft display styles @mentions, #hashtags, URLs, and recognized X entities in action blue; native editing preserves the raw text. Saved Voice-guide quoted real-post examples use the same treatment; ordinary guide prose remains unaffected. Source access belongs in the menu, not inline in the body.
...
Create-agent field labels carry always-visible helper text (the recorded exception above); placeholders are examples only, never instructions — the helper text carries the instruction. Chip fields (X accounts, news websites) render their example placeholder as wrapped overlay text (a native input placeholder cannot wrap), keep an inline + Add control for one-by-one entry, and disable autocapitalize/autocorrect/spellcheck, with the URL keyboard on the websites field. Agent names cap at 30 characters, enforced with a live red under-field error in the login-form style. The connected X-account row shows the green dot and @handle only. Sources-page website chips are two-line — bold publication name over its muted narrow URL — with the same fixed 15px favicon slot as the feed strip.
...
The shared `desk` breakpoint is 700px. Desktop content is at most 1356px wide with 6px gutters and 24px page rhythm; mobile uses 32px gutters and 16px page rhythm, falling back to 16px gutters below 360px wide. Redesigned surfaces use `desk:` responsive gates, never `md:`. Sources keep intentional 8px source-row gaps, 44px mobile and 36px desk row heights, and a 16px offset before each add-source field. Writing Guide masonry uses the shared 16px/24px rhythm for both columns and card separation.
