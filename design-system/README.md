...
What Claude Design gets when the repo is synced. A Claude Code hook (`.claude/hooks/design-sync.sh`) fingerprints the design system and makes Claude re-sync this folder whenever it changes (owner, September 24); `.synced` holds the fingerprint of the last sync. The repo is the source of truth: `DESIGN.md` is the contract, `app/globals.css` holds the live tokens; `tokens.css` here is an export of them and `previews/` shows each piece in light and dark, side by side.
