...
export const testScheduleFrequencies = ["hourly", "daily", "weekly"] as const
...
export type TestScanStreamEvent =
  ...
  | { type: "completed"; items: TestScanItem[]; metrics: TestScanMetrics }
 ...
 */
export function isTestScheduleFrequency(
  value: unknown,
): value is TestScheduleFrequency {
  return (
    typeof value === "string" &&
    testScheduleFrequencies.includes(value as TestScheduleFrequency)
  )
}
...
export function limitScanningInstructions(value: string): string {
  ...
  const limitedLines = normalized
    .split("\n")
    .slice(0, scanningInstructionsMaxLines)
    .join("\n")
 ...
 */
export function getTestScanScheduleError(
...
export function parseTestScanSchedule(value: unknown): TestScanSchedule | null {
  ...
  const weekdays = Array.isArray(value.weekdays)
    ? value.weekdays.filter(isWeekdayValue)
    : []
...
function normalizeTestScanItem(value: unknown): TestScanItem | null {
  ...
  return urls.length > 0 ? { title, body, urls } : null
}
...
export class ScanOutputWriter {
    ...
    switch (event.type) {
      ...
      case "tool_call_input_delta":
        this.inputBuffers.set(
          event.id,
          `${this.inputBuffers.get(event.id) ?? ""}${event.text}`,
        )
        break
  ...
  private writeCompletedOutput(
    ...
    this.write(`${JSON.stringify({ items }, null, 2)}\n`)
    this.write("\n\n=== RESPONSE METRICS ===\n")
    this.write(
      `${JSON.stringify(
        {
