...
**Covers.** The concrete subjects, competitions, clubs, people and event types that count as a story here. Be specific enough to decide a borderline case: name the recurring figures, the rival clubs that appear only as opponents, the tournaments in season. Note which subjects the reporter treats as major versus routine. Every such list is illustrative and must say so in its own words — "recurring subjects include…", not "the recurring subjects are…" — because the model reading this section will otherwise treat your list as the complete set of who qualifies and reject the next new name on the beat. A subject the stated beat plainly covers is on-beat whether or not the corpus has reached it yet.

**Excludes.** What must be filtered out. This is the harder half and the more valuable one. Include both the obvious (promotional and sponsored content, unrelated sport, general news) and — critically — the categories you can see in the corpus that fall outside the stated beat. Exclude by category, never by name: an example may name a person or club only when that subject falls outside the stated beat entirely. Never build an exclusion around a subject the beat covers, however the corpus happens to treat them — a named example becomes a blocklist entry, and every later story about that subject is thrown away even when it lands squarely inside the beat.
...
**The stated beat is the boundary. The corpus is texture inside it.** The reporter told you what they want monitored; that governs. What the corpus adds is precision — which names recur, which rival is a fixture, what "big" looks like on this beat. It does NOT move the boundary, in either direction, and silently moving it is the one failure that makes this section worse than the bare beat string it replaces.

It must not WIDEN the beat. A reporter whose beat is one club, and whose timeline also carries gaming clips and personal posts, has a beat of one club — those other posts are evidence for **Excludes**, not for **Covers**.

It must not NARROW the beat either, and this is the easier mistake to make, because narrowing looks like being specific. A corpus is one sample of one stretch of time: it shows what the reporter happened to file lately, not the limit of what they want to see. Where the stated beat is broader than the corpus, the uncovered ground stays in **Covers** as stated-beat territory, marked "no corpus example; from the stated beat" — never dropped, and never quietly re-described. A reporter who asked for all news about a club has a beat of all news about that club, even if every post you can see is about transfers: transfers are what the corpus shows, not what the beat says. Do not restate a broad beat as the narrower specialty the corpus happens to display, and do not treat a subject area the corpus is silent on as evidence that it is off-beat.
