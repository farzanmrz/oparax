...
Open, each settled inside the slice that needs it: day-zero stories (2); how a story closes and a card updates (2); where polling runs and how often (3); what gets alerted and the bot's transport (5); who may claim a page and account linking (6); the price, the tiers, the size of each pool and how alerts are counted inside a plan (7); the daily ceiling, the ads handle, and whether Farzan and Kush are paying seats or test seats (8).
