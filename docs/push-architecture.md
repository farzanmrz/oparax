...
**Drafting (cap raised to $3/mo, 2026-07-21): the council ships day one.** $3/mo at 50
posts/day = **$2.00 per 1k drafts all-in** — the two-family council + judge fits even with a
cold cache, so the flip-later gate is gone. Reasoning config is part of each member's
identity (a column, not a separate concern):

| Role | Model | Reasoning | $/1k | Basis |
| --- | --- | --- | --- | --- |
| Base drafter | `deepseek/deepseek-v4-flash` + self-check | native adaptive (the tested config) | $1.23 · ~$0.55 warm-cache | measured |
| Second family | `openai/gpt-5-nano` | `low` | ~$0.35 | estimated |
| Judge (picks winner, structured JSON, never writes) | `deepseek/deepseek-v4-flash` | `none`, temp 0 | ~$0.24 | estimated |
| **Ship total** | 2 families + judge | | **$1.82 = $2.73/mo worst case · ~$1.14 = $1.71/mo cached** | **fits unconditionally** |
| Third family (telemetry-gated add) | `zai/glm-4.7-flashx` | `low` | ~$0.41 → trio ~$1.61 = $2.42/mo **cached only** | estimated |

Still dead at $3: `gpt-5.4-nano` (tested, but $1.37 makes the duo $3.27/mo even cached) and
`v4-pro` in any role ($4.07/mo). `qwen3.5-flash` even reasoning-capped (~$0.50) doesn't fit
as a 4th ($3.17/mo cached). The retirement rule governs members: a family whose drafts never
win the judge is dropped; `gpt-5-nano`'s estimate is confirmed or killed by the first weeks
of `getGenerationInfo` telemetry, same as the cache rate that gates the glm add. Latency:
parallel drafts + one judge hop ≈ 10s — nowhere near the 1–2 min allowance.

**Volume flag, restated for $3:** the cap binds per volume — safe at ≤ ~50 tracked
posts/day. At the measured 134/day watch-set floor the allowance is $0.75/1k: the council
fails at any cache state and even solo uncached fails; only warm-cache solo ($0.55) fits.
Approaching that volume forces draft-on-selection or a raised cap — decided by the first
real week of stream data.
