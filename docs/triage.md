...
- **Eve channel auth** — `agent/channels/eve.ts`, a Supabase-cookie AuthFn: nitro-side reassembly of chunked `sb-*` cookies + JWT verification (NOT drop-in `@supabase/ssr` — that stack assumes Next request plumbing). Prerequisite for any deployed chat: until it exists, deployed `/eve/v1/*` 401s browsers and `/dashboard/agents` is localhost-only. Deployed build + public `/eve/v1/health` are already green on `dev` (verified 2026-07-04) — only the authed browser path remains, plus a first real check that project defaults sustain eve's long scan streams (nothing pins `fluid`/durations since `vercel.json` was removed). (from #41)
