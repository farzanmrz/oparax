...
## What this experiment now offers and charges (September 19, 2026)

Recorded here so the experiment and the plan cannot drift apart. The rest of this file below is the September 11 design and has not been rewritten: its dates (a September 30 readout) are dead because the build has not started, and its cohort, thresholds and ceiling have not been re-examined against the product as now defined. The measurement rules and event names still stand.

- **The offer:** type your X handle and one sentence; Oparax recommends at most ten sites and feeds and at least five X accounts, builds oparax.ai/<handle>, watches them, and alerts you in your X DMs. GitHub and Product Hunt digests are added by hand later and are not part of what is tested first.
- **What is metered (owner, September 19):** only watched X posts, as a monthly pool, because X bills half a cent per delivered post. Websites, feeds, GitHub and Product Hunt are not metered; their APIs and feeds are free and each source is fetched once for everyone.
- **The price hypothesis:** the owner's working pitch is about 3,000 watched posts a month with everything else unlimited for about $30, with lower tiers below; this file's September 11 proposal was $29. Neither is decided. Cost is only the floor (about $15 of X cost at full use, $1 to $2 of judging and writing, and alerts on top, which is the part that can sink a plan: a person who wants every update instantly costs several times a once-a-day person at $0.015 a DM). The number to test comes from what creators already pay for tools that feed posts that earn them money (roughly $20 to $100 a month), so the experiment opens at the high end of believable and comes down; hypothesis 2 above ("creators pay the disclosed price") is tested at that number, and the objections heard are recorded word for word.
- **Verified since September 11:** Jev judges Spanish and Catalan items in their own language as well as in English, so nothing is translated first (roadmap section 4). On X's Activity API replies, quotes and reposts of a watched account are all delivered and billed and cannot be filtered out; on the filtered stream they can be excluded before billing (roadmap section 9).
