...
- **Models:** use Vercel AI Gateway as the primary vendor. Start with `spacexai/grok-4.3` for onboarding/discovery and `alibaba/qwen3.7-flash` for relevance judgments, synthesis and cluster/brief decisions. Keep deterministic deduplication ahead of model work. Grok search and the X API are separate acquisition routes with separate metering; native search is not an exhaustive account export or subscription.
- **Search access:** verify X-search tool support and billing on the actual Gateway route with a bounded call. If unavailable, isolate native xAI search behind one adapter while keeping other model calls on Gateway. Persist citations and usage; do not assume model token pricing includes search.
- **Runtime:** retain the Next.js/Vercel web app and Supabase authentication. Use a Railway worker for durable retrieval, processing and notification jobs, with per-source schedules, persisted cursors, retries and job claims. The UI reads persisted results. One scheduled worker can handle the hourly workload without a second Vercel cron scheduler. Multiple workers remain an implementation option if observed load warrants it.
- **Bot:** the repo already has a project-bot client, but its existence is not a successful transport test. Verify an authorized send/reply, opt-in, unsubscribe, payload limits and billing. The screenshot establishes the portal feature exists; the public docs reviewed do not establish its bearer-minting contract. If unavailable, use a dedicated Oparax X account through the documented DM API, subject to recipient permissions and account/app limits. Record transport and failures. Do not substitute another channel and claim the X DM test passed.
...
Run the same onboarding-to-feed pipeline on Liam, Nihan, Farzan, Kush and Reshad. Use known examples to inspect inferred interests, source selection, duplicates and factual attribution; reserve fresh stories for judgment so the evaluation is not merely reproducing examples supplied to the model. Founder-prepared profiles are labeled assisted and cannot prove automatic personalization works. This is an engineering check, not customer validation.
