...
- `lib/web/brightdata.ts` — raw-fetch Bright Data client: `scrapeUrl` (Web Unlocker; block-page detection is status-code based, not body-marker), `pullXTimeline` (Web Scraper API, async trigger/poll/download — the one designated extraction X-read, distinct from the live ingestion stream), Meters `usage_events` (`scrape_web`/`scrape_x_timeline`), never `model_calls`. **`fetchXProfile` and the extraction pre-flight gate it backed are deleted** — live probe 2026-07-25: the X-profile dataset answers the sync `/datasets/v3/scrape` endpoint with `202 + snapshot_id` for a LIVE profile, so the gate read a healthy account as a rejection and blocked every extraction in the product at ~1¢ a go. Don't rebuild it against the sync endpoint; the corpus pull already answers the only question it asked.
