...
The external review council is **`codex` + `grok` + `agy`**, all three. A detach of grok and agy was proposed 2026-07-30 and **reversed the same day**, because the case against each rested on a bug that had already been fixed: agy's TUI picker contamination was patched the day it appeared (`f6dc493` — the picker is now verified against the status line and fails loudly), after which agy returned 10 critiques twice and issue #79 records "agy: passed, 3 findings"; grok's failure history likewise predates its stdout/stderr fix. **Judge a lane on post-fix behaviour, never on its accumulated failure count** — and let a failed lane be reported FAILED rather than deleted.
...
**Other harnesses.** Grok reads `AGENTS.md` natively and finds skills through `.grok/skills → .agents/skills`; its Claude-compat import of rules/agents/hooks is deliberately off, so its context is this file plus whatever a brief hands it. agy discovers essentially nothing here (flat-file skills only, `agy agents` lists none, no rules mechanism) — treat it as a bare model that gets `AGENTS.md` plus its brief. **That is not a handicap for what these two do here.** Both are council critics, and a critic's constraint is context about *this slice*, not tooling: each reads the repo directly, and the `.in.txt` carries the distilled guards. `.agents/agents/code-verifier.md` is the one read-only subagent agy's brief points at.
