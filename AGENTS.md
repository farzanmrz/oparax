...
- **`lib/x/timeline.ts` is the ONE extraction X-read** — 50 most-recent ORIGINAL posts (`MAX_POSTS`), app-only bearer, `exclude=retweets,replies`; replies skew `measuredFacts` toward @handle-openers. Its size also feeds `inferAccountTier`: a smaller corpus can miss the >280-char post proving premium. Handle validation has its own X read.
