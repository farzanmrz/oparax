...
- **Issue #99's measured prompt lab lives in `lab/voice-extractor/filtration-eval/`.** Its frozen no-extractor control is `runs/full-link-enriched-bio`: Qwen 3.7 Flash at temperature 0/medium reasoning receives the raw beat in the user message, a separately translated English source when needed, source handle/profile, production-feasible linked content, and original images or video poster frames, then returns structured `on_beat` + reasoning. It scores 1,252/1,336 (93.71%) on the unanimous FC Barcelona benchmark. Raising both translation and filtration reasoning to high scored 1,247/1,336 (93.34%), so medium remains fixed. Web-search treatments performed worse and are not part of the control. Iterate only `extractor-prompt.md`, inject its filtration guidance into this same control, and compare paired results; do not move the downstream baseline simultaneously.
