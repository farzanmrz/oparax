...
Oparax is an AI news desk for people who follow the news and publish (reporters, creators, newsletter writers): connect an X account and website sources to a desk, and the app watches them, filters what lands on the desk's beat, and drafts posts in the reporter's own voice, measured from their real posting corpus and never hand-authored, surfaced in the app for review. Drafting happens when the reporter presses Draft on a story; posting is always the reporter's decision. The legacy implementation uses Next.js 16 and Supabase, with worker source packages still in the checkout. See the reset baseline below for what is actually deployed.

## Resume here, overnight handoff September 11, 2026

If the owner says "let's continue" or "what were we doing", read [docs/setup-status.md](docs/setup-status.md) first. It contains the completed admin work, all known leftovers, experiment learning/measurement/build status, proposed prices/budgets, constraints and the next-step sequence. Then read [docs/setup-x.md](docs/setup-x.md) and [docs/exp1.md](docs/exp1.md). Basic Memory project `main` (`25f5c0c5-9dcd-4ea1-bc92-3830a613eece`) has [Oparax Experiment 1 - Overnight handoff](https://app.basicmemory.com/notes?id=cloud%7C23ced63c-d463-4082-b0a4-646556de6c41%7C25f5c0c5-9dcd-4ea1-bc92-3830a613eece%7C77bc5055-51b0-4c90-b0b6-b927be7f4e26&explorer=project%3Acloud%7C23ced63c-d463-4082-b0a4-646556de6c41%7C25f5c0c5-9dcd-4ea1-bc92-3830a613eece) (permalink `personal-workspace-3/main/oparax/oparax-experiment-1-overnight-handoff`) and **Oparax Experiment 1 - Credential Index**. Summarize where we stand before resuming; do not ask the owner to retell the conversation.

**Latest unresolved priority: the bot itself must use `@oparax`.** `@oparax_bot` is provisioned with registered encryption keys but is not the accepted final identity. A separate company account pointing at that bot does not satisfy the owner. No rename, Marketplace request, company account or Business subscription has been completed. Investigate the supported handle assignment/transfer path without renaming the owner's personal `@farzanmrz` or buying a plan by assumption.

Current intended services: Vercel, Supabase, X and PostHog. Railway project/workers are deleted; required app credentials have been replaced and stored. **Vercel production is paused.** Historical deployments/previews, parts of Slack/provider cleanup, replacement schema/code removal, new monitoring dashboards and the whole new feature remain pending as detailed in setup-status. Ads are explicitly off-limits until separately instructed. No new `/feature` has been launched; #131 is retired. First finish orientation and the remaining setup decisions, then the owner triggers a fresh Claude Code `/feature` from `beta`.

The code and schema descriptions below describe the legacy checkout, not the target architecture or proof of a running product. Do not recreate its workers, voice/drafting machinery or retired secrets merely to satisfy these descriptions. Both local X docs agents now point to the dedicated X Chat/bot/XDK/key-handling docs; a prior broad claim that those docs were missing was incorrect. Keep actual console observations distinct from documented behavior and tested integration.
