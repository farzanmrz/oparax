...
- **Visual contract:** `DESIGN.md` is the whole design contract (owner, September 23): stock shadcn in the Mira style, installed for real on September 24 (owner's preset `bzq0WEyKe`: Mira, Zinc, a contrast-tuned Oparax blue), dark by default with a light switch, Nunito Sans for headings, Source Sans 3 for text and JetBrains Mono for handles and counts, lucide icons, the reasons beside each choice. The repo is the source of truth and Claude Design holds a synced copy (`/design-sync`, run by the owner) that he designs screens in; the export is the plan's visual contract, from the design brief `/feature` step 1.1 writes. Every UI skill has three jobs: it shapes the plan, guides the build and checks the result.
