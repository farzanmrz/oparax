...
**Two Slack paths exist and share nothing.**

1. `SLACK_WEBHOOK_URL` → a workspace-wide webhook into the owner's `#drafts`: a personal observation rig, and in code the legacy fallback whenever a desk has no `slack_accounts` row. Always works, no login.
2. The per-desk OAuth app (`SLACK_CLIENT_*`) → the end user's own linked Slack, the real product delivery path.

A verification slice PROVES the delivery hop when a message lands in `#drafts` via the fallback. **A missing per-desk Slack link is not a delivery blocker and never a reason to gate a run on the owner performing OAuth** — conflating the two cost ~12% of one session. The Slack app's redirect allowlist still has no `https://beta.oparax.ai/auth/slack/callback` entry: dashboard config, not app code.
...
- `lib/voice/` — the voice pipeline. Four load-bearing facts:
  ...
  - `deploy-guide.ts` strips extractor-verification sections before a guide becomes a drafting prompt (16.1% off every draft) and reroutes `## Beat & Scope` to the drafter's `beatSpec`.
  - `corpus-store.ts` upserts and **never prunes** — a re-extraction only adds and refreshes.
  - `extraction-run.ts`'s `startRun` is an **atomic claim returning a boolean**; callers must not spend when it returns false. It bounds one desk to ONE concurrent run so a double-clicked Retry bills once. It is **not** per-reporter/per-day rationing and must never grow into it. **Progress reaches the browser by POLLING an ownership-proving server action, never Supabase Realtime** — the table is deny-all RLS.
...
- **Connect X is a UI gate, not a security boundary.** `createDesk` derives `reporter_handle` from the linked `x_accounts` row, so through the form a user can only extract their own voice — a *product* rule, not an RLS one. `experiments` has an owner-scoped INSERT policy with no value constraint, so any signed-in user can mint a row with any handle. **Never argue that relaxing the form gate widens a security boundary; there is no boundary there to widen.** Two facts make it safe: extraction reads the public corpus with the app-only bearer and never touches the reporter's OAuth token, and posting resolves the account via `getXAccount(ownerId)` — owner-keyed. Voice and publishing were never coupled in code, only in the form.
...
The external review council is **`codex` + `grok` + `agy`**, all three, all headless CLIs launched in the background. A fourth, **manual** lane exists for GUI-only surfaces: `.agents/workflows/critique-plan.md` becomes `/critique-plan` in Antigravity, reads the `.feature/critique-*.in.txt` the plan phase already wrote, and writes `.feature/critique-manual.out.md` back. No wrapper, no tmux — the same durable-file handoff that makes the Codex↔Claude phase hops work. It is owner-triggered only; no phase ever waits on it.

 A detach of grok and agy was proposed 2026-07-30 and **reversed the same day**, because the case against each rested on a bug that had already been fixed: agy's TUI picker contamination was patched the day it appeared (`f6dc493` — the picker is now verified against the status line and fails loudly), after which agy returned 10 critiques twice and issue #79 records "agy: passed, 3 findings"; grok's failure history likewise predates its stdout/stderr fix. **Judge a lane on post-fix behaviour, never on its accumulated failure count** — and let a failed lane be reported FAILED rather than deleted.
...
**Other harnesses.** Grok reads `AGENTS.md` natively and finds skills through `.grok/skills → .agents/skills`; its Claude-compat import of rules/agents/hooks is deliberately off, so its context is this file plus whatever a brief hands it.

**Antigravity is a family of surfaces, and they do NOT share discovery — never generalize a measurement from one to another.** `.agents/rules/` and `.agents/workflows/` are Google's documented *workspace* customization dirs (markdown, 12k chars each; rules support always-on / glob / @-mention activation; a workflow becomes the slash command `/<filename>`), and the `.agents/rules → .claude/rules` symlink exists so the GUI surfaces pick this repo's rules up. **The `agy` CLI (1.1.8) honours none of it** — probed 2026-07-30 with an always-on canary rule in `.agents/rules/`, and it answered `NONE`; it exposes no rules/workflows/skills subcommand, and `agy agents` lists nothing. So the council lane, which runs the CLI, gets `AGENTS.md` plus its brief and nothing else.

**That is not a handicap for what the council does.** A critic's binding constraint is context about *this slice*, not tooling: it reads the repo directly and the `.in.txt` carries the distilled guards. `.agents/agents/code-verifier.md` is the one read-only subagent agy's brief points at.
