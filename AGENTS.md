...
- **Visual contract:** `DESIGN.md` is the design contract. Owner, September 23: the custom dark system it describes is being replaced by stock shadcn light and dark from a preset code (dark by default, one accent, the logo kept), in a small issue before slice 1, after which `DESIGN.md` shrinks to the preset, the fonts, the logo files, the breakpoint and the reasons; the repo is the source of truth and Claude Design holds a synced copy (`/design-sync`) that the owner designs screens in. Until that issue ships, the existing tokens still bind. Screens are designed by the owner in Claude Design from the design brief `/feature` step 1.1 writes; the export is the plan's visual contract. Every ui skill has three jobs: it shapes the plan, guides the build and checks the result.
