...
`.env.local`, nineteen keys (table below), plus `SENTRY_AUTH_TOKEN` in its own gitignored `.env.sentry-build-plugin`; Supabase dashboard-side config (unrelated to the other two keys): `.claude/rules/supabase.md`. Frontend test login: `testuser@oparax.ai` / `hello123`.
