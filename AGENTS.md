...
- **Model budget.** No manual model switching, ever — run sessions on Opus. Fable is reached only through the Fable-pinned `planner` agent (`/feature` Phase 1 dispatches it; on a Fable usage-limit failure the flow re-dispatches it on Opus and says so at the gate). Every other dispatched agent pins Opus or Sonnet via frontmatter or dispatch param.
