...
docs/                   references/cogs.md (every cost of serving a person), references/decisions.md (every locked and rejected decision with its reason), references/state.md (the handoff: what is done, pending and open), roadmap.md (the plan), onboarding-algorithm.md (the algorithm), source-table-seed.json (the shared table's seed), exp1.md (experiment), setup.md (external setup), findings.md, reshad.md, people.tsv (discovery)
