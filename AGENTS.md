...
- **Global outside counsel:** `/counsel` in Claude Code and `$counsel` in Codex ask a fixed set of outside models, each through its own command-line tool (Astra, Gemini Pro, Grok, Opus and Fable by default, Cursor models and others by name), for independent advice on any question and report the positions side by side; owner-invoked only; installed at `~/.agents/skills/counsel` outside the repository. Distinct from critique, which hunts defects under a findings contract.
